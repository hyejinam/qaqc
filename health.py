...
if df is not None:
    ...
    if not selected_animal:
        ...
        with col1:
            st.image(r"C:\Users\남혜지\OneDrive\cowow.png", width=500)
            st.markdown(
    ...
    if selected_farm in optimal_variables:
        ...
        for idx, (var, (min_val, max_val)) in enumerate(optimal_vars.items()):
            if var in avg_values:
                ...
                cols[idx % 3].write(f"<p style='font-size: 30px;'>최적범위: {min_val} ~ {max_val}</p>", unsafe_allow_html=True)
                cols[idx % 3].write(f"<p style='font-size: 30px; color: {text_color};'>농장 평균값: {original_value:.2f}</p>", unsafe_allow_html=True)
                if warning_msg:
